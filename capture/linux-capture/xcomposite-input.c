...
struct dstr xcomp_window_name(xcb_connection_t* conn, Display* disp, xcb_window_t win)
{
    ...
    if (name->type == ATOM_STRING)
    { // Latin-1, safe enough
        dstr_ncopy(&ret, data, xcb_get_property_value_length(name));
        free(name);
        return ret;
    }
    if (name->type == ATOM_TEXT)
    { // Default charset
        char* utf8;
    ...
    if (name->type == ATOM_COMPOUND_TEXT)
...
void watcher_register(xcb_connection_t* conn, struct xcompcap* s)
{
    ...
    if (xcomp_window_exists(conn, s->win))
    {
        ...
        da_push_back(watcher_registry, (&(struct reg_item)
        {
            s, s->win
        }));
    }
...
static obs_properties_t* xcompcap_props(void* data)
{
    ...
    if (s)
    {
        ...
        if (had_window_saved)
        {
            ...
            da_push_back(window_strings, (&(struct WindowInfo)
            {
                name_lower, name, desc
            }));
        }
        else
        {
