...
class HylaranaCoreService : Service() {
    ...
    fun createSender(
        ...
        observer: Observer
    ): Pair<MediaStreamDescription, Int>? {
        ...
        return sender?.let { it.getDescription() to it.getPort() }
    }

    fun createReceiver(surface: Surface, configure: HylaranaReceiverConfigure, observer: Observer) {
        val audioConfig = Audio.getAudioCodecConfigure()
        receiver = HylaranaService.createReceiver(configure, object : HylaranaReceiverObserver() {
    ...
    class ServiceBinder(private val service: HylaranaCoreService) : Binder() {
        ...
        fun createReceiver(
            ...
            service.createReceiver(surface, configure, observer)
        }
